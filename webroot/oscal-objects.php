...
class OSCAL {
	...
	// ----------------------------------------------------------------------------
	// Runs an XSLT 1.0 transformation against the OSCAL object
	//     - returns the results of the transformation
	//     
	public function Transform($xslt_file) {
		$this->messages->Debug("** Transform (" . $xslt . "):");
		$ret_val = false;
		global $oscal_roots;

		$xsl = new DOMDocument;
		if (! $xsl->load(OSCAL_FORM_FILES . $xslt_file) === false) { // Load the file and only proceed if there were no errors.
			$proc = new XSLTProcessor();
			$proc->importStyleSheet($xsl);
			
			$output = $proc->transformToXML($this->dom);
			if ($output === false) {
				$output = "[Processing Error]";
				$this->messages->Debug("Could not transform. Problem applying XSLT to XML.");
				$status = false;
			}
		} else{
			$this->messages->Debug("Could not open XSLT file.");
			$status = false;
		}
		return $output;
	}	
	...
	// ----------------------------------------------------------------------------
	// Runs an XSLT 3.0 transformation against the OSCAL object
	//     - returns the results of the transformation
	// This uses Saxon HE for more advanced transformations. The code is similar 
	//     to the XML to JSON code.
	// It calls Java externally to run Saxon. The resulting transformation is 
	//     delivered as a text file, which must be open and read.
	public function Transform3($xslt_file) {
		$this->messages->Debug("** Transform (" . $xslt . "):");
		$temp_file_name = com_create_guid() . ".tmp";
		$ret_val = "";
		global $oscal_roots;
	...
	//	$file_to = FriendlyPath($file_to);
		...
		$ExOutput = array();
		$result = "";
		$ExString = 'java -jar ' . SAXON_HE . ' -s:"' . $this->file_name . '" -o:"' . OSCAL_FORM_FILES . $temp_file_name . '" -xsl:"' . OSCAL_FORM_FILES . $xslt_file . '" ' . $flags . '  2>&1';
		$this->messages->Debug("RUNNING: " . $ExString);
		exec($ExString, $ExOutput, $result);
		// **** Check $ExOutput for "Warning" or "Error" messages, and log to debugger.

		$transform_result = file_get_contents(OSCAL_FORM_FILES . $temp_file_name)  ;
		if ($transform_result === false) {
			$this->messages->Debug("ERROR READING: " . $temp_file_name);
		} else {
			$ret_val = $transform_result;
		}
